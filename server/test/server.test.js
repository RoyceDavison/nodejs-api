...
describe("GET /todos/:id", () => {
  ...
  it("should not created a todo doc created by other user", (done) => {
    request(app)
...
describe("DELETE /todos/:id", () => {
  ...
  it("should not remove a todo if it is created by other", (done) => {
  ...
  it("Should reject invalid login", (done) => {
    request(app)
      ...
      .end((err, res) => {
        ...
        User.findById(users[1]._id)
          .then((user) => {
            expect(user.tokens.length).toBe(1); //之前是0，后来加了tokens变成1
            done();
          })
          .catch((e) => done(e));
      });
